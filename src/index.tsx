...
const getLibraries = callable<[ServerConfig], { success: boolean; libraries: MusicLibrary[]; message?: string }>("get_libraries");
...
function ServerFormPage({ existingServer }: { existingServer?: ServerConfig | null }) {
  ...
  const [libraryKey, setLibraryKey] = useState(existingServer?.library_key || "");
        ...
        <>
          ...
          {serverType === "plex" && (
            <PanelSection title="Music Library">
              <PanelSectionRow>
                <div style={{ ...type.meta, color: theme.onSurfaceVariant, padding: "0 2px 8px" }}>
                  Plex registers audiobooks as a music library too. Pick one to keep
                  audiobooks out of search and playback.
                </div>
              </PanelSectionRow>
              <PanelSectionRow>
                <ButtonItem
                ...
                >
                  <div style={{
                    ...
                    <FaList style={{ fontSize: "12px" }} />
                    {isLoadingLibraries ? "Loading…" : libraries.length ? "Refresh libraries" : "Load libraries"}
                  </div>
                </ButtonItem>
              </PanelSectionRow>
              ...
              {libraries.length > 0 && (
                <>
                  <PanelSectionRow>
                    <RowButton
                      onClick={() => setLibraryKey("")}
                      actionDescription="Use every library"
                      accent={!libraryKey ? theme.primary : undefined}
                    >
                      <div style={{
                        width: "22px", height: "22px", borderRadius: theme.radiusFull, flexShrink: 0,
                        background: !libraryKey ? theme.primary + "22" : "transparent",
                        border: `1px solid ${!libraryKey ? theme.primary : theme.outline}66`,
                        display: "flex", alignItems: "center", justifyContent: "center",
                      }}>
                        {!libraryKey ? <FaCheck style={{ fontSize: "9px", color: theme.primary }} /> : null}
                      </div>
                      <RowText title="All libraries" subtitle="Previous behaviour" />
                    </RowButton>
                  </PanelSectionRow>
                  {libraries.map((lib) => {
                    const selected = libraryKey === lib.key;
                    return (
                      <PanelSectionRow key={lib.key}>
                        <RowButton
                          onClick={() => setLibraryKey(lib.key)}
                          actionDescription={`Use ${lib.title}`}
                          accent={selected ? theme.primary : undefined}
                        >
                          <div style={{
                            width: "22px", height: "22px", borderRadius: theme.radiusFull, flexShrink: 0,
                            background: selected ? theme.primary + "22" : "transparent",
                            border: `1px solid ${selected ? theme.primary : theme.outline}66`,
                            display: "flex", alignItems: "center", justifyContent: "center",
                          }}>
                            {selected ? <FaCheck style={{ fontSize: "9px", color: theme.primary }} /> : null}
                          </div>
                          <RowText title={lib.title} />
                        </RowButton>
                      </PanelSectionRow>
                    );
                  })}
